 ...
 * @return mixed
 */
if( !function_exists( 'et_get_ban_expired_period' )) {
...
/**
 * Cron job for automatically unbar user
 */
if( !function_exists( 'et_setup_schedule_unban' ) ) {
    function et_setup_schedule_unban() {
        if( !wp_next_scheduled( 'et_cron_unban_users' ) ) {
            wp_schedule_event( time(), 'hourly', 'et_cron_unban_users' );
        }
    }
}
add_action( 'wp', 'et_setup_schedule_unban' );

/**
 * Search expired banned users and unban them
 * @return void
 */
if( !function_exists( 'et_unban_expired_users') ) {
    function et_unban_expired_users() {
        global $wpdb;
        $user = AE_Users::get_instance();

        $user_query = new WP_User_Query( array(
            'meta_key' => $user->meta_ban_expired,
            'meta_value' => date( get_option( 'date_format' ) ),
            'meta_compare' => '<'
        ) );
